...
static bool isInputCompliesAddmmCudaLt(
  ...
  return (beta.toComplexDouble() == 1.0
    ...
    && (
      // NOTE: the full condition is either we can use a bias ptr in epilogue,
      // or copy bias into result and use only activation epilogue.
      // Conditions for bias ptr are already checked in `can_use_epilogue_bias`.
...
bool launchGemmAndEpilogueCublasLt(
  ...
  return at::cuda::blas::gemm_and_bias<scalar_t, res_scalar_t>(
    ...
    /*pre_activation_ptr==*/epilogue_params.opt_pre_activation.has_value()
      ? epilogue_params.opt_pre_activation.value().data_ptr<res_scalar_t>()
      : nullptr
  );
}
